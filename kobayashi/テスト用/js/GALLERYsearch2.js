document.addEventListener('DOMContentLoaded', function () {
  ...
  searchButton.addEventListener('click', function () {
    ...
    const searchTerm = searchInput.value.trim(); // 入力されたテキストを取得
    ...
    if (searchTerm) {
      ...
      // 他のキーワードに対応するクラスを追加
      if (!isMatched) {
        // 該当する素材がない場合、メッセージを表示
        searchResults.textContent = '該当する素材はありませんでした';
      } else {
        // 該当する素材がある場合、メッセージをクリア
...
function hideAllClasses() {
  const classList = ['.you', '.mari', '.yada', '.au', '.kiri'];
